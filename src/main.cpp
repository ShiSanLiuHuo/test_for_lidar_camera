...
void timer()
{
	while(running)
	{
		...
		int count = frameCount.exchange(0);
		std::cout << "frame:" << count << "FPS" << std::endl;
	}
	
}

void sinalHandler(int signum)
{
...
int main()
{
    ...
    std::signal(SIGINT,sinalHandler);
    ...
    // CameraGetImageResolution(hCamera,&res);
    // res.iIndex = 0xff;
    // res.iWidth = 4096;
    // res.iHeight = 3072;
    // res.iWidthFOV = 4096;
...
*/
    ...
    while(running)
    {
        if(CameraGetImageBuffer(hCamera, &sFrameInfo, &pbyBuffer, 2000) == CAMERA_STATUS_SUCCESS)
        {
            ...
            // // Bayer 原始数据转 cv::Mat
            // cv::Mat bayerImg(height, width, CV_8UC1, pbyBuffer);

            // // 创建 RGB8 图像
            // cv::Mat rgbImg;
            
            // // 根据不同的 Bayer 模式选择转换代码
            // cv::cvtColor(bayerImg, rgbImg, cv::COLOR_BayerBG2RGB); // 视情况更改为 COLOR_BayerRG2RGB, COLOR_BayerGB2RGB 等
